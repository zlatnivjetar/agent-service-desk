...
function OverviewPageContent() {
  ...
  const comparisonOverviewQuery = useDashboardOverview(comparisonParams, {
    enabled: isInternal && compareMode && comparisonBounds != null,
    refetchInterval: autoRefreshSeconds > 0 ? autoRefreshSeconds * 1000 : false,
  })
  const watchlistQuery = useDashboardWatchlist(overviewParams, {
    enabled: isInternal,
    refetchInterval: autoRefreshSeconds > 0 ? autoRefreshSeconds * 1000 : false,
  })
  ...
  function clearFilters() {
    router.push(`${pathname}?range=30d`)
  }
  ...
  function applySavedView(state: Record<string, unknown>) {
    router.push(hrefFromState(pathname, state))
  }
  ...
  const watchlistItems = watchlistQuery.data?.watchlist_items ?? []
  const tickets = ticketsQuery.data?.items ?? []
  const totalPages = ticketsQuery.data?.total_pages ?? 1
  const kpis = overview?.kpis
  const comparisonOverview = comparisonOverviewQuery.data
